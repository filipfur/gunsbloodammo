...
Menu::Menu(std::vector<const char*> menuItems,
	   std::vector<const char*> helpItems,
	   std::vector<const char*> optionItems,
	   std::vector<const char*> highscoreItems,
...
void Menu::draw(SDL_Renderer &renderer){
  SDL_SetRenderDrawColor(&renderer, _bgColor.r, _bgColor.g,
			 _bgColor.b, _bgColor.a);
  ...
  for(int i = 0; i < itemAmount; ++i){
    if(i == 0){//Title
      if(!(text_surface = TTF_RenderText_Shaded(_titleFont,
						_currentItems.at(i),
						_titleColor1, _bgColor))){
	std::cerr<<TTF_GetError()<<std::endl;
      }
    }
    else if(i == _currentSelection){
      if(!(text_surface = TTF_RenderText_Shaded(_highlightFont,
						_currentItems.at(i),
						_highlightColor, _bgColor))){
	std::cerr<<TTF_GetError()<<std::endl;
      }
    }
    else{
      if(!(text_surface = TTF_RenderText_Shaded(_breadFont,
						_currentItems.at(i),
						_breadColor, _bgColor))){
	std::cerr<<TTF_GetError()<<std::endl;
      }
    }
  ...
  if(_subMenu == 2){ //Options
    ...
    if(image == NULL){
      ...
      SDL_SetTextureColorMod(texture, _crosshairColor.r,
			     _crosshairColor.g, _crosshairColor.b);
